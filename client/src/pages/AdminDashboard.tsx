...
export default function AdminDashboard() {
  ...
  const [alertForm, setAlertForm] = useState({ crop: 'Rice', type: 'Health', severity: 'high', message: '', action: '' });
  ...
  return (
    <div style={{ padding: '40px', maxWidth: '800px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button onClick={logout} style={{ padding: '8px 16px', background: '#333', color: 'white', border: 'none', cursor: 'pointer' }}>Logout</button>
      </div>

      <div style={{ background: 'white', padding: '30px', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', marginTop: '20px' }}>
        <h3>Create New Regional Alert</h3>
        <form onSubmit={handleRaiseAlert} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          
          <div style={{ display: 'flex', gap: '15px' }}>
            <div style={{flex: 1}}>
              <label>Target Crop</label>
              <select value={alertForm.crop} onChange={e => setAlertForm({...alertForm, crop: e.target.value})} style={{ width: '100%', padding: '10px', marginTop: '5px' }}>
                <option>Rice</option><option>Wheat</option><option>Cotton</option>
              </select>
            </div>
            <div style={{flex: 1}}>
              <label>Severity Level</label>
              <select value={alertForm.severity} onChange={e => setAlertForm({...alertForm, severity: e.target.value})} style={{ width: '100%', padding: '10px', marginTop: '5px' }}>
                <option value="high">High (Red)</option><option value="medium">Medium (Orange)</option><option value="low">Low (Yellow)</option>
              </select>
            </div>
          </div>
          ...
          <button type="submit" style={{ background: '#d32f2f', color: 'white', border: 'none', padding: '15px', fontSize: '1.1rem', cursor: 'pointer', marginTop: '10px' }}>
            BROADCAST ALERT
          </button>
        </form>
      </div>
